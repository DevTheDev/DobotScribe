function returnMsg = drawLetter(L, dobot, allLetters, camera, firstTime)
    ...
    if(~firstTime)
      ...
      moveToStart(dobot, camera); % This is a wrapper on the moveToPose command
    else
      %Move to the paper and start drawing (doesn't really matter where)
      %NOTE: Needs to be tuned.
      moveToPose([100 20 0], dobot);
    end
    %Loop
    for P = LetterPaths{1};
        drawSegment(P, dobot); %Another wrapper on the moveToPose command
    end
